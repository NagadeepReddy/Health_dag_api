...
/* Collapse multiple wallets to one budget per VAST ID.
   We validated the 23 duplicate VAST IDs:
   each has 2 wallets but the same $100,000 budget. */
wallet_vast AS (
  SELECT
    vast_id,
...
/* Historical completed weekly consumption */
weekly_spend AS (
  SELECT
    DATE_TRUNC(
      spend_date,
...
/* Compare each week with its previous week */
...
SELECT
  ...
  ROUND(
    wa.avg_weekly_spend_usd,
    2
  ) AS avg_weekly_spend_usd,

  ROUND(
    ma.avg_monthly_spend_usd,
    2
  ) AS avg_monthly_spend_usd,
